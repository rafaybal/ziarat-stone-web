...
const WhyChooseUsSection: React.FC = () => {
  ...
  return (
    <section className="py-20 bg-marble-darkbrown text-white relative overflow-hidden">
      {/* Enhanced background with marble pattern overlay */}
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-40 -left-40 w-80 h-80 bg-white opacity-5 rounded-full blur-3xl animate-pulse-light"></div>
        <div className="absolute -bottom-40 -right-40 w-80 h-80 bg-white opacity-5 rounded-full blur-3xl animate-pulse-light" style={{animationDelay: '1.5s'}}></div>
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[200%] h-32 bg-gradient-to-r from-white/0 via-white/5 to-white/0 rotate-45 animate-shimmer"></div>
      </div>

      <div className="container mx-auto px-4 relative z-10">
        <h2 className="text-3xl md:text-4xl font-bold mb-12 text-center animate-fade-in font-serif">
          Why Choose Us
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {benefits.map((benefit, index) => (
            <div 
              key={benefit.id} 
              className="bg-marble-brown bg-opacity-30 rounded-lg p-6 hover:bg-opacity-40 transition-all duration-500 transform hover:-translate-y-2 hover:shadow-lg hover:shadow-black/20 opacity-0 animate-fade-in backdrop-blur-sm border border-white/10 group"
            ...
            >
              <div className="bg-white rounded-full w-12 h-12 flex items-center justify-center mb-4 group-hover:animate-bounce-gentle shadow-lg relative overflow-hidden">
                <Check className="h-6 w-6 text-marble-brown relative z-10" />
